...
Terrain.prototype.act = function() {
    ...
    //     this.mesh[g+1] = octaveNoise(this.mesh[g], this.mesh[g+2], 4, 2);
    // }
...
Terrain.prototype.setHoneycomb = function() {
    this.rate = 4;
    for(let z =  -this.rate*Math.sqrt(2)-this.partition; z < this.rate*Math.sqrt(2)+this.partition; z += this.partition) {
        let layer = [];
        for(let x = -this.rate*Math.sqrt(2)-this.partition; x < this.rate*Math.sqrt(2)+this.partition; x += this.partition) {
            let coordX = x;
            let coordY = octaveNoise(x, z, 4, 1.05);
            let coordZ = z;
...
let fragmentShaderSource = `
    ...
    void main() {
        ...
        vec4 color = texture2D(u_image, vec2(v_horiz, 1.0-v_depth));
        vec4 depth_color = vec4(color.rgb, 1.0); //->[0, 0.2]
        vec4 far_color = vec4(depth_color.rgb, depth_color.a-pow(sqrt(exp(v_far)-1.0), 1.5)/5000.0); //->(sqrt(e^x)-1)^2
...
function drawScene(gl) {
    ...
    let perspective = matrices["perspective"](sliders[9].value, gl.canvas.width/gl.canvas.height, 0.001, 200);
